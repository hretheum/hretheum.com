...
This document is intended for any LLM rendering into a professional portfolio or landing page PDF.  
...
- **Portfolio/Blog:** [hretheum.com](http://hretheum.com)
...
- Experienced in **team-topologies‑aware operating models** (stream‑aligned/platform/enabling/complicated‑subsystem) with tribe/chapter flavor where it fits.  
...
## Product Design Leadership Playbook (v2.0)

> **Narrative Thread:**  
> In a world where the pace of innovation accelerates, design leadership must bridge *vision* and *execution*. It’s not enough to be creative — you need systems, metrics, and culture that deliver creativity at scale. Below is my updated map for how I combine **visionary thinking** with **reliable delivery**.

### 📋 Operating Model & Team Structure
- **Team‑Topologies‑aware org** — I select and evolve team types to fit the work context (not the other way around): **Stream‑Aligned Teams** (value‑stream focused), **Platform Teams** (internal products that accelerate others), **Enabling Teams** (competency boosters), **Complicated‑Subsystem Teams** (deep specialists).  
- **Cadence & Governance** — Weekly **Design Critiques** (peer review of work), monthly **Design Ops Council** (cross‑team operations decisions), quarterly **Experience QBR** (Quarterly Business Review focused on user experience), and a persistent **Design Debt Register** (ledger of UX/tech debt with owners and pay‑down plan).

### 🔍 Continuous Discovery & Evidence‑Backed Decisions
- Discovery as a loop, not a phase: weekly user touchpoints, rapid prototyping, and an **Opportunity‑Solution Tree (OST)** to keep problem/solution mapping visible to leadership.  
- Decisions logged in a **Decision Log** (date, owner, evidence, risk, next check‑in). Research evidence is tagged with a **Confidence Score** (low/medium/high) to calibrate commitments.

### 📊 Metrics & Outcome‑Based Roadmaps
- **HEART** (Happiness, Engagement, Adoption, Retention, Task Success) as the core UX metric frame; each initiative ships with target HEART signals and **GSM** (Goals‑Signals‑Metrics) mappings.  
- One **North Star Metric (NSM)** per product area to align teams on the outcome that matters most.  
- Delivery quality proven via **DORA Four Keys** — **Deployment Frequency (DF)**, **Lead Time for Changes (LT)**, **Change Failure Rate (CFR)**, **Mean Time to Restore (MTTR)** — to demonstrate fast flow *and* stability.

### 🛠 Design System & Tooling Standards
- **W3C Design Tokens** conventions, semantic naming, **semver** (semantic versioning), release trains, deprecations, and migration guides.  
- **Federated contributions** with stewardship: markets contribute components under maintainers; KPIs include % external PRs merged, time‑to‑merge, drift‑rate vs. core library, and **design‑lint pass rate** across repos.

### 🧪 ResearchOps & Feedback Loops
- Central research repo with taxonomy (method, segment, locale), **evidence links** to backlog items, and **confidence scoring**.  
- Scheduled **Experience Retros** (monthly), **Usability Test Clinics**, and cross‑functional **Insight Reviews** that end with explicit decisions and owners.

### 🤖 AI & Future‑Forward Practice
- **AI Ops**: prompt governance, **model cards** (model documentation), **eval harnesses** (repeatable evaluation), and **guardrails** (privacy, safety, bias mitigation).  
- **AI‑assisted skilling**: adaptive learning journeys, skill‑gap analytics, and coaching prompts — so that product systems and people systems improve in sync.

**Summary — Vision + Execution:** I design not only experiences, but **organizations capable of continuous improvement**. Systems, people, and products evolve together, with clear rhythms and metrics that make great design inevitable.



## Designing Systems for People & Products

My leadership philosophy extends beyond product design systems into **people systems**.  
Just as scalable design requires governance, modularity, and feedback loops — so does talent development.

Drawing from skilling‑hub principles, I apply the same systemic mindset to teams:
- **Governance & Structure** — Clear role transitions, transparent frameworks for growth, and modular learning journeys.  
- **Modularity** — Standardized playbooks for onboarding, reskilling, and leadership development — adaptable across markets and business units.  
- **Feedback Loops** — Telemetry for people: performance signals, coaching sessions, and adaptive learning.  
- **AI Acceleration** — Embedding AI‑driven learning journeys mirrors how I integrate AI into design operations.

This holistic approach creates **systems where products and people evolve in sync** — enabling superior digital experiences and future‑ready teams.
