...
- **< 1 second** P50 time to first meaningful tokens in local dev.
- **Zero flicker** above-the-fold: assistant loads minimized with SSR-safe shell.
- **Session continuity** via `thread_id` + turn index stored in `sessionStorage`.

## FLOW OVERVIEW

```mermaid
flowchart TD
  A[User] -->|types| B[Chat widget]
...
- If `intent_confidence < threshold` → show 3 brand/industry‑aware suggested queries (CSR).
- If `hesitation > 2s` on primary CTA → show reassurance tooltip (CSR).
