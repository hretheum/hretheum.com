...
export default async function BrandPage(props: { params: Promise<{ slug?: string }>; searchParams: Promise<{ preview?: string }> }) {
  const resolvedParams = await props.params
  ...
  // Log preview access
  if (isPreview) {
    console.log('[preview] Campaign preview accessed:', {
      slug,
  ...
  return (
    <>
      ...
      {isPreview && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-yellow-100 border-b-2 border-yellow-400 px-4 py-2 flex items-center justify-between shadow-md">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-yellow-700">Not indexed • Not cached</span>
          </div>
          <a
            href={`/admin?tab=campaigns&edit=${slug}`}
            className="px-3 py-1 bg-yellow-600 text-white rounded-md text-sm font-medium hover:bg-yellow-700"
          >
            ← Edit in Admin
          </a>
        </div>
