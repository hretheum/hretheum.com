"use client";
// Client-safe versions of campaign components for MDXRemote rendering in development.
...
export function CaseStudyRich(props: { title: string; context?: string; role?: string; challenge?: string; approach?: string[]; outcome?: string }) {
  ...
  return (
    <section className="my-10">
      ...
      {role && (
        <p className="mt-2 text-neutral-700"><span className="font-medium" style={{ color: 'var(--campaign-accent)' }}>Role:&nbsp;</span>{role}</p>
      )}
      {challenge && (
        <p className="mt-2 text-neutral-700"><span className="font-medium" style={{ color: 'var(--campaign-accent)' }}>Challenge:&nbsp;</span>{challenge}</p>
      )}
      ...
      {outcome && (
        <p className="mt-2 text-neutral-700"><span className="font-medium" style={{ color: 'var(--campaign-accent)' }}>Outcome:&nbsp;</span>{outcome}</p>
      )}
    </section>
  )
}
...
export function SectionTitle({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="not-prose py-10 text-center">
      <h2 className="font-black text-gray-900 leading-[0.95] tracking-tight mb-2" style={{ fontSize: 'clamp(2rem, 8vw, 6rem)' }}>
        {title}
      </h2>
      {subtitle && (
    ...
    <section className="not-prose relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen min-h-[60vh] md:min-h-screen flex items-center justify-center overflow-x-hidden bg-black text-white mt-16 mb-0">
      ...
      <div className="text-center z-10 px-4 sm:px-6 max-w-6xl">
        ...
        <div className="inline-block text-left md:text-center p-6 md:p-8" style={{ backgroundColor: 'var(--campaign-accent)', color: '#000' }}>
          ...
          <a href="https://linkedin.com/in/eofek" target="_blank" rel="noopener noreferrer" className="text-base md:text-lg underline" style={{ color: '#000' }}>
