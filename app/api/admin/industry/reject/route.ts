...
export async function POST(req: NextRequest) {
  try {
    ...
    const svc = getSvc()
    const { error } = await svc.from('brand_industry_suggestions').update({ dismissed: true }).eq('id', id)
    if (error) return NextResponse.json({ error: 'update_failed' }, { status: 500, headers: { 'X-Robots-Tag': 'noindex, nofollow' } })
